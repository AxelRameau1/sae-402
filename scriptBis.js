let tileSize = 64; // taille d'une tuile en pixels

let tileImages = []; // tableau pour stocker les images des tuiles
let spriteX, spriteY; // position du sprite
let spriteSpeed = 2; // vitesse du sprite
...
function setup() {
  ...
  createCanvas(map[0].length * tileSize, yScreenSize, /*"pixelated x1.5"*/);
...
//Fonction de fin
function win() {
  if (/*scrollingPositionY >= -3000*/ scrollingPositionY >= 0){
    //freeze le jeu
    cameraSpeed = 0; 
    playerSpeed = 0; 
    pufferFishSpeed = 0; 
    ...
    //Afficher texte
    textSize(64); // taille du texte
    fill(255, 255, 255); // couleur
    textAlign(CENTER, CENTER); // alignement
    text("Gagné !", width/2, defeatLine - 500); // position
...
function mapLoading() {
  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
      ...
      if (tileType > 0 && tileType <+ 28) { // ne dessine pas la tuile si elle correspond à un ennemi
        tilePosition = [];
...
function playerMovement() {
  ...
  if (finish == false) {
      //console.log(mouse.y);
      player.moveTowards(mouse.x, mouse.y, .06);
  }*/
...
function defeat () {
  if (player.position.y > defeatLine || finish == true ) {
    ...
    fill(255, 255, 255); // couleur
    textAlign(CENTER, CENTER); // alignement
    text("Perdu !", width/2, defeatLine - 500); // position
